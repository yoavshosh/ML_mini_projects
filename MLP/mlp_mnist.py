...
class KerasMnist(object):
    ...
    def build_model_skip(self):
        ...
        layers = []    
        current_layer = Dense(self.hidden_layer_dims[0], activation='relu')(x)
        layers.append(current_layer)
        for i in range(1,len(self.hidden_layer_dims)):                
            if i%self.skips == 0:
                current_layer = Dense(self.hidden_layer_dims[i])(layers[-1])
                current_layer = keras.layers.add([current_layer,layers[i-self.skips]])
                current_layer = Activation('relu')(current_layer)
                layers.append(current_layer)
            else:    
                current_layer = Dense(self.hidden_layer_dims[i], activation='relu')(layers[-1])
                layers.append(current_layer)
        out = Dense(self.num_classes, activation='softmax')(layers[-1])
